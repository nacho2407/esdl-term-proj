/*
  ...
  ��� ���
  1) ADC + DMA + TIM2(2kHz Ʈ����)�� �������� ���ø� (Circular Buffer)
  2) Ʈ����(PC4) �� TRIG,seq,ambient ���� + ���� ����
  3) PC ACK(���� seq) ���� �� 120ms ������ ���ø� ����
  4) DMA ���ۿ��� 120ms ����(min ADC = peak) �м� �� hit ���� �� RESULT ����
  5) PC STATUS �޽����� ���������� �߰�
  6) ���¸ӽ�: IDLE (���) / WAIT_ACK (�߻� ��ȣ ����) / SAMPLING (ȭ�� �÷��� ���� ��)
 */
...
#include <stdlib.h>
 ...
 /* ==========================================================================
  ...
  // DMA�� ���� �� ������ ���� ���� ( ���� 256�� ����)
#define ADC_BUF_LEN             256                   //512�� �÷��� �Ǳ� ��

// PC �ø��� RX ����
#define RX_BUF_SIZE             128

// ADC ���ø� ���ļ� (TIM2�� 2kHz Ʈ����)    -> TIM2�� 2kHz ���ø� (0.5ms ����) �Ѵٰ� ����
...
// ���� ���� �Ӱ谪 (peak(ADC ��)�� �� ������ ������ hit=1)
#define HIT_THRESHOLD           1000                             // Ʃ���ϱ�!!!!!! (��ο���� ���� ŭ)



typedef enum {
    SHOT_IDLE = 0,     // �ƹ� �͵� �� �ϴ� ����
    SHOT_WAIT_ACK,     // TRIG ���� �� PC���� ACK ��ٸ��� ��
    SHOT_SAMPLING      // ACK ���� �� 120ms ���ø� ��
} ShotState;

volatile uint32_t msTicks = 0;                  // SysTick millis
volatile uint16_t adc_buf[ADC_BUF_LEN];         // ADC ���� �����ϴ� DMA circular ����

volatile uint32_t sequence_num = 0;        // ��ü �� ������
volatile uint32_t current_shot_seq = 0;        // ���� ó�� ���� �� ��ȣ  >> TRIG/ACK/RESULT ��� �̰� ������� �� 

volatile uint8_t  flag_trigger = 0;        // Ʈ���� �߻� (ISR �� main)  , EXTI ���ͷ�Ʈ���� 1�� ����. main������ Ʈ���� ó��
volatile uint8_t  flag_ack_matched = 0;        // �̹� ���� ���� ACK ����, ACK ����, seq�� ���� �� 1�� ���� 
volatile uint8_t  flag_pc_msg_ready = 0;        // USART1 ISR���� �� �� ���� �Ϸ�Ǹ� 1
volatile ShotState shot_state = SHOT_IDLE;     // ���� �� ����

// ���ø� ����
uint32_t sampling_start_time = 0;              // ACK ���� ������ miilis(). 120ms �� ����
uint16_t sampling_start_index = 0;              // ACK ���� ���� DMA write index   -> �� �ε������� 240 ���� ��ĵ
uint16_t min_peak_value = 4095;           // ���ø� �������� �ּ� ADC (���� ���� ��)

// PC RX ����
char usart1_rx_buf[RX_BUF_SIZE];     //PC���� ���� ���ڿ��� �����ϴ� ����/�ε���
volatile uint8_t usart1_rx_idx = 0;    // ISR���� �ѱ��ھ� ��ٰ� \n ������ ���ڿ� �ϼ�.
    ...
    while (1)
    {
        ...
        if (shot_state == SHOT_SAMPLING)
        {
            if ((millis() - sampling_start_time) >= SAMPLE_WINDOW_MS) // 120ms �������� ���ø� ���� ���� ������ ��
            {
                ...
                for (uint16_t i = 0; i < samples; i++)        // DMA ���� ���� ��ĵ
                {
                    uint16_t idx = (sampling_start_index + i) % ADC_BUF_LEN; // ���� ���� �ε��� ���
                    uint16_t v = adc_buf[idx];   // �� ���� �� �а�
                    if (v < min_peak_value)      // �ּҰ� ����
                        min_peak_value = v;     
                }
...
void System_Init_All(void)
{
    ...
    TIM2_Config();      // ADC Ʈ���ſ� 2kHz Ÿ�̸�
    ADC_DMA_Config();   // �������� ADC+DMA
    EXTI_Configure();   // Ʈ���� ��ư
    NVIC_Configure();   // ���ͷ�Ʈ �켱����
}
...
void RCC_Configure(void)
{
    ...
    // ADC Ŭ��= PCLK2(72MHz)/6 = 12MHz �� ����   >> F1 ADC �ִ� Ŭ�� (14MHz) ���Ϸ� ���߱� ���ؼ�
...
void TIM2_Config(void)
{
    ...
    TIM_OCInitStructure.TIM_Pulse = 500;  // ��Ƽ 50%
...
void EXTI_Configure(void)
{
    ...
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling; // ȸ�ο� �°� ����
