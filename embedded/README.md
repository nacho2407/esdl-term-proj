...
3.  **SAMPLING (계측 상태)**
      ...
      * 임계값 비교 후 `RESULT` 메시지(명중 여부)를 PC로 전송합니다.

-----

## 4\. 튜닝 포인트 (현장 설정)

`main.c` 파일 상단의 매크로 값을 수정하여 현장 상황에 맞게 감도를 조절할 수 있습니다.

```c
// 명중 판정 임계값 (0 ~ 4095)
// 이 값보다 ADC 값이 낮으면(밝으면) 명중으로 판단합니다.
...
```
  ...
  * 튜닝 
      * 스마트폰 플래시 등을 이용해 명중 시의 센서 값(예: 300)과 평상시 값(예: 3000)을 확인한 후, 그 중간값으로 `HIT_THRESHOLD`를 설정

-----

## 5\.  프로젝트 설명 (발표용 요약)

> "이번 임베디드 시스템은 **DMA와 타이머 트리거**를 활용해 **2kHz의 고속 정밀 샘플링**을 구현했습니다. PC의 응답(ACK)을 기준으로 과거 120ms 데이터를 분석하기 때문에 통신 딜레이가 있어도 정확한 명중 판정이 가능합니다. 또한, PC의 게임 상태 정보를 **블루투스로 실시간 중계**하여 모바일 앱과 연동됩니다."
